...
<html>
  <head>
    <title>Estore---在线支付</title>
  </head>
  
  <body>
        <form action="${pageContext.request.contextPath}/PayServlet"  method="post">
        ${pageContext.request.contextPath}/PayServlet
      	<table width="60%">
			<tr>  
				<td bgcolor="#F7FEFF" colspan="4">
			 		订单号：${param.id }<input type="hidden" name="orderId" value="${param.id}"/> 
			 		支付金额：${param.money} <input type="hidden" name="money" value="${param.money}"/>元
			 	</td>
			</tr>
			<tr><td><br/></td></tr>
			<tr>
			...
			<tr>
			  <td><INPUT TYPE="radio" NAME="pd_FrpId" value="ICBC-NET-B2C">招商银行 </td>
